...
function card() {
  ...
  axios.get('http://localhost:3000/menu').then(function (response) {
    response.data.forEach(function (obj) {
      new MenuCard(obj.img, obj.alt, obj.title, obj.description, obj.price, ".menu .container", "menu__item").render();
    });
  });
}
...
function form() {
  ...
  var modalTimer = setTimeout(openModal, 50000);
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (e) {
      ...
      // })
      // .then(data => data.text()) 
      postData('http://localhost:3000/requests', JSON.stringify(obj)).then(function (data) {
        console.log(data);
  ...
  function showThanksModal(message) {
    var prevModalDialog = document.querySelector('.modal__dialog'); // mavjud dialog oyna

    prevModalDialog.classList.add('hide'); //  hide class yordamida dispaly = none
    openModal(); // yangi modal ochish

    var thanksModal = document.createElement('div'); // dialog uchun div yaratish
...
function slider() {
  ...
  nextBtn.addEventListener('click', function () {
    console.log(offsetX);
    if (offsetX == extractNumber(width) * (slides.length - 1)) offsetX = 0;else offsetX += extractNumber(width);
    slidesField.style.transform = "translateX(-".concat(offsetX, "px)");
    indexSlides.innerHTML = getZero(offsetX / extractNumber(width) + 1);
  });
  prevBtn.addEventListener('click', function () {
    if (offsetX == 0) offsetX = extractNumber(width) * (slides.length - 1);else offsetX -= extractNumber(width);
    slidesField.style.transform = "translateX(-".concat(offsetX, "px)");
    indexSlides.innerHTML = getZero(offsetX / extractNumber(width) + 1);
  });
}
...
function tab() {
  // Tabs
  var tabheaderItems = document.querySelector(".tabheader__items");
  var tabcontents = document.querySelectorAll(".tabcontent");
  loadTabs();
  tabheaderItems.addEventListener("click", function (event) {
    // console.dirp(event.target);

    tabheaderItems.childNodes.forEach(function (child) {
      if (child.nodeType == 1) child.classList.remove("tabheader__item_active");
    });
  ...
  function loadTabs() {
    tabheaderItems.childNodes.forEach(function (child) {
      if (child.nodeType == 1) if (child.classList.contains("tabheader__item_active")) tabcontentsActivator(child.textContent.toLocaleLowerCase());
    });
  }
}
...
function timer() {
  ...
  function getTimeEnd(endTime) {
    ...
    if (timer <= 0) {
      ...
      days = Math.floor(timer / (1000 * 60 * 60 * 24)), hours = Math.floor(timer / (1000 * 60 * 60) % 24), minutes = Math.floor(timer / (1000 * 60) % 60), seconds = Math.floor(timer / 1000 % 60);
    }
    return {
      timer: timer,
      days: days,
...
/* harmony import */ var _modules_loader__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ../modules/loader */ "./src/modules/loader.js");
/* harmony import */ var _modules_card__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../modules/card */ "./src/modules/card.js");
